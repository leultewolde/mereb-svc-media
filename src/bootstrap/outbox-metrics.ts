...
import type { MediaOutboxStatusCounts } from '../adapters/outbound/prisma/media-prisma-asset-repository.js';
...
const meter = metrics.getMeter('svc-media-outbox-relay');
...
let depthSnapshot: MediaOutboxStatusCounts = {
  pending: 0,
  processing: 0,
  published: 0,
  failed: 0,
...
export function setMediaOutboxQueueDepth(counts: MediaOutboxStatusCounts): void {
  depthSnapshot = counts;
}

export function recordMediaOutboxFlushMetrics(input: OutboxFlushMetricsInput): void {
